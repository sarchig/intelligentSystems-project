...
- Perform CLASS-WISE stratified split (8:1:1) or (7:1:2) so that each class is present in train/val/test.
...
from tensorflow.keras.applications import (
    VGG16, DenseNet121, Xception, ResNet50V2
)
...
OUTPUT_ROOT = Path("outputs/train_runs_Scenario3")
...
def apply_model_preprocessing(inputs, model_name: str):
    if model_name == "VGG16":
        return layers.Lambda(vgg16_preprocess, name="preprocess_vgg16")(inputs)
    if model_name == "DenseNet121":
        return layers.Lambda(densenet_preprocess, name="preprocess_densenet121")(inputs)
    if model_name == "Xception":
        return layers.Lambda(xception_preprocess, name="preprocess_xception")(inputs)
    if model_name == "ResNet50V2":
        return layers.Lambda(resnet50v2_preprocess, name="preprocess_resnet50v2")(inputs)
    raise ValueError(model_name)
...
def build_model(model_name: str, num_classes: int) -> keras.Model:
    ...
    x = apply_model_preprocessing(inputs, model_name)
